...
export function Dialog({ open, onOpenChange, children }: DialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      />
      <div className="z-50 grid w-full max-w-lg gap-4 border border-[var(--color-rule)] bg-white p-6 shadow-lg sm:rounded-lg animate-in fade-in-90 zoom-in-95">
        {children}
        <button
          onClick={() => onOpenChange?.(false)}
...
export function DialogDescription({ className, ...props }: React.HTMLAttributes<HTMLParagraphElement>) {
  return <p className={cn("text-sm text-[var(--color-ink-2)]", className)} {...props} />
}
...
export function DialogContent({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn("grid gap-4 py-4", className)} {...props} />
}

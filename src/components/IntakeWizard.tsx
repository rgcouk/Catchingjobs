...
export default function IntakeWizard({
  ...
  const validateStep2 = () => {
    ...
    if (formData.hasRightToWork === null) {
      newErrors.hasRightToWork =
        'You must declare your Right to Work status for statutory compliance.';
    }
          ...
          {authState === 'unauthenticated' && (
            ...
            >
              <div className="text-center space-y-2">
                ...
                <h4 className="text-lg font-bold text-slate-900 tracking-tight">Join Our Team</h4>
                <p className="text-xs text-slate-600 max-w-sm mx-auto leading-normal">
                  To secure your position on our harvesting squads in{' '}
                  <span className="text-slate-900 font-bold">
                    {regionName !== 'all' ? regionName : 'your county'}
                  </span>
                  , sign up instantly with your social provider.
                </p>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-md p-3 flex gap-2.5 items-center text-[11px] text-slate-600 leading-normal">
                ...
                <p>
                  By signing in, we instantly retrieve your public name and email to fast-track your
                  work credentials. On first sign-in, you'll complete a brief 2-step onboarding
                  questionnaire.
                </p>
              </div>
                    ...
                    <div className="space-y-4">
                      <div className="text-center space-y-1">
                        ...
                        <p className="text-[10px] text-slate-400">
                          to continue to{' '}
                          <span className="font-semibold text-slate-600">CatchingJobs.co.uk</span>
                        </p>
                      </div>
                      ...
                      {!showCustomAuthForm ? (
                        <div className="divide-y divide-slate-100 border border-slate-200 rounded-md overflow-hidden bg-slate-50">
                          {googlePresets.map((preset, idx) => (
                            ...
                            >
                              <div
                                className={`w-8 h-8 rounded-full ${preset.avatarBg} text-white flex items-center justify-center font-bold text-xs font-mono shrink-0`}
                              >
                                {preset.name
                                  .split(' ')
                                  .map((n) => n[0])
                          ...
                          >
                            <span className="w-8 h-8 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center text-sm font-bold shrink-0">
                              +
                            </span>
                        ...
                        <form
                          ...
                          className="space-y-3 p-3 border border-slate-200 rounded-md bg-slate-50"
                        >
                  ...
                  {authProvider === 'facebook' && (
                    <div className="space-y-4">
                      <div className="text-center space-y-1">
                        ...
                        <h5 className="font-bold text-slate-800 text-[13px]">
                          Log in with Facebook
                        </h5>
                        <p className="text-[10px] text-slate-500 leading-normal max-w-xs mx-auto">
                          CatchingJobs requests access to your public profile parameters (name &
                          avatar) and email.
                        </p>
                      </div>
                            ...
                            >
                              ...
                              <div className="truncate text-xs">
                                <p className="font-bold text-slate-800">
                                  Continue as {preset.name.split(' ')[0]}
                                </p>
                                <p className="text-slate-400 font-mono text-[10px]">
                                  {preset.email}
                                </p>
            ...
            >
              <div className="bg-emerald-50 border border-emerald-200 rounded-md p-3.5 flex gap-2.5 shadow-sm text-xs text-slate-700 leading-normal">
                ...
                <div>
                  ...
                  <p className="mt-0.5">
                    We successfully imported your details from{' '}
                    <strong>{authProvider === 'google' ? 'Google' : 'Facebook'}</strong>. Let's
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-md p-3.5 flex items-center gap-3.5">
                <div
                  className={`w-10 h-10 rounded-full ${socialProfile?.avatarBg || 'bg-slate-800'} text-white flex items-center justify-center text-sm font-bold font-mono shrink-0`}
                >
                  {socialProfile?.name
                    .split(' ')
                    .map((n) => n[0])
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <span
                      className={`text-[8px] font-mono font-medium ${formData.hasRightToWork === true ? 'text-slate-300' : 'text-slate-400'}`}
                    >
                      Share Code / Passport
                    </span>
                  </button>
                  ...
                  >
                    ...
                    <span
                      className={`text-[8px] font-mono font-medium ${formData.hasRightToWork === false ? 'text-slate-300' : 'text-slate-400'}`}
                    >
                      No active work visa
                    </span>
                  </button>
                ...
                {errors.hasRightToWork && (
                  <p className="text-[10px] text-red-500 font-mono">{errors.hasRightToWork}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <span className="font-bold text-xs">YES</span>
                    <span
                      className={`text-[8px] font-mono font-medium ${formData.hasDrivingLicense === true ? 'text-slate-300' : 'text-slate-400'}`}
                    >
                      Clean UK standard
                    </span>
                  </button>
                  ...
                  >
                    <span className="font-bold text-xs">NO</span>
                    <span
                      className={`text-[8px] font-mono font-medium ${formData.hasDrivingLicense === false ? 'text-slate-300' : 'text-slate-400'}`}
                    >
                      No vehicle license
                    </span>
                  </button>
                ...
                {errors.hasDrivingLicense && (
                  <p className="text-[10px] text-red-500 font-mono">{errors.hasDrivingLicense}</p>
                )}
              </div>
            ...
            >
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-md p-4 space-y-1">
                ...
                <p className="text-[10px] text-slate-500 leading-snug">
                  ...
                  <strong className="text-slate-800">{socialProfile?.name}</strong>.
                </p>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200 p-4 rounded-md text-left space-y-3">
                ...
                <ul className="text-xs text-slate-650 space-y-3">
                  <li className="flex items-start gap-3">
                    <span className="font-bold text-slate-800 font-mono bg-slate-200 w-5 h-5 rounded-full flex items-center justify-center text-[10px] shrink-0 mt-0.5">
                      1
                    </span>
                    <div>
                      <p className="font-semibold text-slate-800">Coordinator Call Back</p>
                      <p className="text-[10px] text-slate-500 leading-normal">
                        Our team is notified. A coordinator will contact you at{' '}
                        <strong>{formData.phone}</strong> to verify details and shifts.
                      </p>
                    </div>
                  </li>
                  <li className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 leading-normal">
                        We will send you safety training resources and a link to log into the{' '}
                        <strong className="text-slate-800">Safety Culture App</strong> to complete
                        your digital onboarding check-sheets.
                      </p>
                    </div>
                  </li>
                  <li className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 leading-normal">
                        Once tasks are completed on Safety Culture, you are unlocked on our local

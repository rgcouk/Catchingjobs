...
export function KanbanBoard({ columns, tasks, onTaskStatusChange, onTaskSelect }: KanbanBoardProps) {
  return (
    <div className="flex flex-1 h-full gap-4 overflow-x-auto pb-4">
      {columns.map((col) => (
        <Card key={col.id} className="w-80 shrink-0 flex flex-col bg-muted/30 border-border shadow-none">

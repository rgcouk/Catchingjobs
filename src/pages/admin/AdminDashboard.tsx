  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      case 'kanban':
        ...
        return (
          <div className="p-6">
            <h1 className="text-2xl font-bold font-display text-[var(--color-ink)] mb-4">Applications Kanban</h1>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white p-4 rounded-lg shadow-sm border border-[var(--color-rule)]">
                <h3 className="font-semibold text-lg border-b border-[var(--color-rule)] pb-2 mb-4">New / Submitted</h3>
                <div className="space-y-3">
                  {newApps.map(app => (
                    <div key={app.id} className="p-3 bg-slate-50 border border-[var(--color-rule)] rounded shadow-sm">
                      <p className="font-medium text-sm">{app.firstName} {app.lastName}</p>
                      <p className="text-xs text-slate-500">{app.jobPosting?.title || 'Unknown Job'}</p>
                      <button onClick={() => updateApplicationStatus(app.id, 'REVIEWING')} className="mt-2 text-xs text-[var(--color-accent)]">Move to Review</button>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-white p-4 rounded-lg shadow-sm border border-[var(--color-rule)]">
                <h3 className="font-semibold text-lg border-b border-[var(--color-rule)] pb-2 mb-4">In Review / Interview</h3>
                <div className="space-y-3">
                  {reviewApps.map(app => (
                    <div key={app.id} className="p-3 bg-slate-50 border border-[var(--color-rule)] rounded shadow-sm">
                      <p className="font-medium text-sm">{app.firstName} {app.lastName}</p>
                      <p className="text-xs text-slate-500">{app.jobPosting?.title || 'Unknown Job'}</p>
                      <button onClick={() => updateApplicationStatus(app.id, 'HIRED')} className="mt-2 text-xs text-[var(--color-accent)]">Mark as Hired</button>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-white p-4 rounded-lg shadow-sm border border-[var(--color-rule)]">
                <h3 className="font-semibold text-lg border-b border-[var(--color-rule)] pb-2 mb-4">Hired</h3>
                <div className="space-y-3">
                  {hiredApps.map(app => (
                    <div key={app.id} className="p-3 bg-slate-50 border border-[var(--color-rule)] rounded shadow-sm">
                      <p className="font-medium text-sm">{app.firstName} {app.lastName}</p>
      ...
      case 'locations':
        return (
          <div className="p-6">
            <h1 className="text-2xl font-bold font-display text-[var(--color-ink)] mb-4">Location Manager</h1>
            <div className="bg-white p-6 rounded-lg shadow-sm border border-[var(--color-rule)] mb-6">
              ...
              <form onSubmit={handleLocationSubmit} className="space-y-4 max-w-md">
                <div>
                  ...
                  <input name="id" required className="w-full border rounded px-3 py-2" placeholder="e.g. norfolk-region" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Name</label>
                  <input name="name" required className="w-full border rounded px-3 py-2" placeholder="e.g. Norfolk" />
                </div>
                <div>
                ...
                <button type="submit" className="bg-[var(--color-accent)] text-white px-4 py-2 rounded">Create Location</button>
              </form>
            </div>
              ...
              <ul className="space-y-4">
                {locations.map(region => (
                  <li key={region.id} className="border p-4 rounded bg-slate-50">
                    <strong className="text-lg">{region.name} ({region.id})</strong>
                    <ul className="ml-4 mt-2 list-disc list-inside">
                      {region.towns?.map((town: any) => (
                        <li key={town.id}>{town.name} ({town.id})</li>
                      ))}
                    </ul>
                  </li>
                ))}
      ...
      case 'jobs':
        ...
        return (
          <div className="p-6">
            <h1 className="text-2xl font-bold font-display text-[var(--color-ink)] mb-4">Job Manager</h1>
            <div className="bg-white p-6 rounded-lg shadow-sm border border-[var(--color-rule)] mb-6">
              ...
              <form onSubmit={handleJobSubmit} className="space-y-4 max-w-md">
                <div>
                  ...
                  <input name="title" required className="w-full border rounded px-3 py-2" placeholder="e.g. Chicken Catcher" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Description</label>
                  <textarea name="description" required className="w-full border rounded px-3 py-2" rows={3} />
                </div>
                <div>
                ...
                <button type="submit" className="bg-[var(--color-accent)] text-white px-4 py-2 rounded">Create Job</button>
              </form>
            </div>
              ...
              <ul className="space-y-2">
                {jobs.map(job => (
                  <li key={job.id} className="border-b pb-2">
                    ...
                    <p className="text-sm text-slate-600">Town ID: {job.townId} | Status: {job.status}</p>
                  </li>
                ))}
              </ul>
            </div>
      ...
      case 'settings':
        return (
          <div className="p-6">
            <h1 className="text-2xl font-bold font-display text-[var(--color-ink)] mb-4">Admin Settings</h1>
            <div className="bg-white p-6 rounded-lg shadow-sm border border-[var(--color-rule)]">

...
export default function JobShareModal({
  ...
  return (
    ...
    >
      <div
        className="bg-white border border-[#E2E8F0] rounded-md max-w-md w-full shadow-2xl overflow-hidden p-6 space-y-5 animate-in zoom-in-95 duration-200"
      ...
      >
        ...
        <div className="flex items-start justify-between">
          ...
          <button
            onClick={onClose}
            className="text-[#94A3B8] hover:text-[#0F172A] p-1.5 rounded-md hover:bg-slate-100 transition-colors cursor-pointer"
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              readOnly
              value={jobUrl}
              className="flex-1 bg-[#F8FAFC] border border-[#E2E8F0] rounded-md px-3 py-2 text-xs font-mono text-[#334155] focus:outline-none select-all"
            />
            <button
              onClick={handleCopyLink}
              className={`px-3.5 py-2 rounded-md font-mono text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer ${
                copied
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {shareChannels.map((channel) => (
              <a
                ...
                className={`flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-md text-xs font-medium transition-colors shadow-xs ${channel.color}`}
              >
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-md text-xs font-medium bg-[#F1F5F9] hover:bg-[#E2E8F0] text-[#0F172A] border border-[#CBD5E1] transition-colors cursor-pointer"
            >
        ...
        {/* QR Code expansion */}
        {showQr && (
          <div className="pt-2 flex flex-col items-center justify-center p-4 bg-[#F8FAFC] border border-[#E2E8F0] rounded-md text-center space-y-2 animate-in fade-in duration-150">
            <img
              src={qrImageUrl}
              alt={`QR Code for ${jobTitle}`}
              className="w-36 h-36 border border-white rounded-md shadow-xs"
              loading="lazy"
            />

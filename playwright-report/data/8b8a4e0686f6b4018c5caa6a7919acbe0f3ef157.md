...
- Name: m2_challenger_verification.spec.ts >> Milestone 2 Empirical Challenge Verification >> 3. Interactive Navigation Transitions & Routing Flow >> CH-M2-005: 404 fallback routing and recovery transition to National Hub
- Location: tests/m2_challenger_verification.spec.ts:240:5

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/chickens/nonexistent-corridor-999
Call log:
  - navigating to "http://localhost:3000/chickens/nonexistent-corridor-999", waiting until "domcontentloaded"
...
> 243 |       await page.goto('/chickens/nonexistent-corridor-999', { waitUntil: 'domcontentloaded' });
  ...
  288 | 
  289 |         // 2. Serialized initialData present in head script
  290 |         expect(html).toContain('id="__INITIAL_DATA__"');
  291 |         expect(html).toContain(townName);
  292 | 
  293 |         // 3. Schema.org JobPosting structured data pre-rendered
  294 |         expect(html).toContain('"@type":"JobPosting"');
  295 |         expect(html).toContain(`"addressLocality":"${townName}"`);
  296 |         expect(html).toMatch(/"addressRegion":".*"/);
  297 | 
  298 |         // 4. Invariant: No loading placeholder in SSR wire HTML
  299 |         expect(html).not.toContain('Loading regional context...');
  300 |       });
  301 |     }
  302 |   });
  303 | });
